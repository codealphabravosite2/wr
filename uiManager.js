...
export class UIManager {
  ...
  updateToggleContent(toggleId, newContent) {
    ...
    toggleSection(toggleId) {
        ...
        if (this.scrollRestoreTimeout) {
            ...
            this.scrollRestoreTimeout = null;
            this.isRestoring = false; // reset lock so next toggle isn't blocked
        }
  ...
  _handleToggleSection(toggleId) {
    ...
    try {
        ...
        // If the section was just opened, jump straight to edit mode
        if (toggle.isOpen) {
            requestAnimationFrame(() => {
  ...
  _attachViewModeListeners(viewDiv, toggleId) {
    // Desktop: mouseup works reliably
    viewDiv.addEventListener('mouseup', () => {
      setTimeout(() => this._checkSelectionInDiv(viewDiv, toggleId), 10);
    });

    // Mobile: selectionchange fires AFTER the selection is finalized,
    // unlike touchend which fires before. Debounce to avoid rapid-fire checks.
    ...
    // Attach selectionchange when user touches this div, remove when focus leaves
    viewDiv.addEventListener('touchstart', () => {
      document.addEventListener('selectionchange', onSelectionChange);
    }, { passive: true });

    // Clean up selectionchange listener when touch ends without selection
